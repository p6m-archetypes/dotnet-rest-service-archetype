...
if context:get("persistence") == "PostgreSQL" then
    require("dotnet-resource-postgresql").render(context, dest)
...
if context:get("messaging") == "Kafka" then
    require("dotnet-resource-kafka").render(context, dest)
elseif context:get("messaging") == "Pulsar" then
    require("dotnet-resource-pulsar").render(context, dest)
end

if context:get("has_s3") then
    require("dotnet-resource-s3").render(context, dest)
end

if context:get("has_azure_blob") then
    require("dotnet-resource-azure-blob").render(context, dest)
end
