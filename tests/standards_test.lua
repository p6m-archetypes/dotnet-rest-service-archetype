...
---
--- Container-first (S8): the SUT is the archetype's own .platform/docker/local image on a topology
--- network — docker is the only requirement, no .NET SDK on the host.
	...
	prova.group(label, { requires = { "docker" }, tags = { "standards" } }, function(g)
		p6m.standards.api(g, sut, {
			persisted = function(t, name, count)
				local svc = t:use(sut)
				t:expect(
					svc.db.client:query_value([[SELECT count(*) FROM "Items" WHERE "DisplayName" = $1]], { name }),
